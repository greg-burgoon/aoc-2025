...
function getRollsFromPaperMatrix(paperRollMatrix) {
    ...
    let newPaperMatrix = markMatrix.map(row => row.map(cell => cell < 4 || cell >= 100 ? '.' : '@'));
    return [ sum, newPaperMatrix ];
}

let sum = 0
let [result, newPaperMatrix] = getRollsFromPaperMatrix(paperRollMatrix);
sum += result;
while (result > 0) {
    [result, newPaperMatrix] = getRollsFromPaperMatrix(newPaperMatrix);
    sum += result
}
